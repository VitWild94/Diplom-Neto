...
class Bot:
    ...
    def input_looking_age(self, user_id, age):
        ...
        try:
            ...
            age_to = int(a[1])
            if age_from == age_to:
                self.message_send(user_id, f' Ищем возраст {self.naming_of_years(age_to, False)}')
                return
            self.message_send(user_id, f' Ищем возраст в пределах от {age_from} и до {self.naming_of_years(age_to, True)}')
            return
        except IndexError:
            age_to = int(age)
            self.message_send(user_id, f' Ищем возраст {self.naming_of_years(age_to, False)}')
            ...
            self.message_send(user_id, f' NameError! Введен не правильный числовой формат!')
            return
        except ValueError:
            self.message_send(user_id, f' ValueError! Введен не правильный числовой формат!')
            return
    ...
    def looking_for_persons(self, user_id):
        ...
        res = self.vk_user_got_api.users.search(
            ...
            count=1000,
            fields="can_write_private_message, "  # Возможные значения: 1 — может; 0 — не может.
                   "city, "  
                   "domain, "  
                   "home_town, "  
        )
    ...
    def show_found_person(self, user_id):
        ...
        if self.get_found_person_id() == None:
            ...
            self.message_send(user_id, self.found_person_info(self.get_found_person_id()))
